...
program RJ_MCMC
    ...
    real sigmav,sigmav_old,sigmav_new,AR_birth_old   !proposal on vsv when Birth move
...
50      tes=.true.
    ...
    write(100,*)burn_in,nsample,widening,thin
    write(100,*)d_min,d_max
    write(100,*)width
    write(100,*)xi_min,xi_max
    write(100,*)vp_min,vp_max
...
!             endif
        ...
        do i=1,npt
            if (abs(voro(i,4))>0.4) then
                !if (out==0) write(*,*)'too big vp - refused'
                !if (out==1) then
                write(*,*)'too big vp - accepted'
                write(*,*)'rank',rank
                write(*,*)'previous depths',voro(:npt,1)
                write(*,*)'proposed depths',voro_prop(:npt,1)
                write(*,*)'previous vp',voro(:npt,4)
                write(*,*)'proposed vp',voro_prop(:npt,4)
                write(*,*)'proposed vp_data',vp_data(noc+1:nptfinal)
                stop "too big vp"
                !endif
            endif
        enddo
...
!
        IF (ount.GT.burn_in) THEN
            ...
            IF (mod(ount,thin)==0) THEN
                ...
                write(100,*)vp_data(nptfinal:1:-1)
                write(100,*)like_w
...
!                     enddo
                ...
                if (mod(th_all,everyall)==0) then
                    close(100)
                    write(filenamemax,"('/All_models_invert_',I3.3,'_',I3.3,'.out')") rank, sample/everyall/thin
                    open(100,file=dirname//filenamemax,status='replace')
                    write(100,*)rank,sample/everyall,everyall
                    write(100,*)burn_in,nsample,widening,thin
                    write(100,*)d_min,d_max
...
!                     avvp(i)=avvp(i)+interp(r(j-1),r(j),vp_data(j-1),vp_data(j),d)*alpharef
!                     avvp_wide(i)=avvp_wide(i)+interp(r(j-1),r(j),vp_data(j-1),vp_data(j),d)
!
...
!                     avvp_alt(i,:)=avvp_alt(i,:)+interp(r(j-1),r(j),vp_data(j-1),vp_data(j),d)*alpha
!
!                     if (abs(interp(r(j-1),r(j),xi(j-1),xi(j),d)-1.)>0.01) then
...
!                     ind_vp=ceiling((interp(r(j-1),r(j),vp_data(j-1),&
!                         vp_data(j),d)-vpvsv_min)*disv/(vpvsv_max-vpvsv_min))
...
!                     if (ind_vp>disv) then
!                         write(*,*)"vp>disv"
!                         write(*,*)interp(r(j-1),r(j),vp_data(j-1),vp_data(j),d),vpvsv_min,vpvsv_max
!                         write(*,*)voro(:,4)
...
!     k=0
!     if (th_all.ne.0) then !normalize averages
!         avvs=avvs/th
!         avvp=avvp/th
!         avxi=avxi/th
