...
c      real,intent(in):    r(Nmax),rho(Nmax),vpv(Nmax),vph(Nmax),vsv(Nmax),vsh(Nmax),qkappa(Nmax),qshear(Nmax),eta(Nmax)
      ...
      integer,parameter :: mk=350, ifdeck=1
      integer,intent(in) :: ifanis2,N2,nic2,noc2,jcom2
      integer,intent(in) :: lmin2,lmax2,nmin2,nmax2
      integer :: lmin,lmax,nmin,nmax
      real,dimension(mk),intent(in) :: r2,rho2,vpv2,
...
c*** find roots by spline interpolation ***
      ...
      real,dimension(nmodes_max),intent(inout) :: c_ph,period,raylquo
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,wgrav,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      common/mtab/we(2),de(2),ke(2),wtry,bm
