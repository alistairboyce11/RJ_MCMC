    ...
    for i_dir in range(len(directories)):
        ...
        for widening in widenings_in:
            ...
            if rank in np.unique(computing):
                print('sending',rank)
                comm.Send([alpha_max_tmp,MPI.FLOAT],dest=0,tag=1+int(widening)*100+int(cluster)*1000)
                comm.Send([np.array([num_models_tmp]),MPI.INT],dest=0,tag=3+int(widening)*100+int(cluster)*1000)
                print('sent',rank)
            if rank==0:
...
print('start')
params_dispersion, dispersion_ref, dispersion_all=postprocess_util.get_dispersion(directories,clusters,points=points)
...
if rank==0:
    ...
    for function in output:
        filename='processing_'+'_'+function+'_outputs.h5'
        print(filename)
        f = h5py.File(outdir+'/'+filename,'w')
